...
#define LWS_INITIAL_TXBUFFER LWS_MAX_SOCKET_IO_BUF  // transmit buffer grows as needed
...
{
    const UriQueryListA *q = find_query(wss, "policy");
    if (q == NULL) {
	      ...
	      void *in, size_t len)
{
    ...
    case LWS_CALLBACK_FILTER_PROTOCOL_CONNECTION:
	{
	    ...
	    char geturi[LWS_MAX_HEADER_LEN];
	    char uriargs[LWS_MAX_HEADER_LEN];
	    int retval = 0;
